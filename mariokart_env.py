...
class MarioKartEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if lap_fraction >= self.next_checkpoint:
            checkpoint_reward = self.base_checkpoint_reward * (lap_fraction ** 2)
        ...
        obs = np.array([self.current_progress], dtype=np.float32)
    ...
    def reset(self):
        ...
        self.current_progress = 1.0
        self.next_checkpoint = self.checkpoint_gap
        return np.array([self.current_progress], dtype=np.float32)
